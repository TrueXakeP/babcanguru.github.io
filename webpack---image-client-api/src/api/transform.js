...
function getTransform(fittingType, src, target, options) {
    const isSEOBot = imageServiceUtils.isSEOBot(options)
    ...
    const isWEBPSupport = !isSEOBot && imageServiceUtils.isWEBPBrowserSupport(fileType)
    const devicePixelRatio = isSEOBot ? 1 : imageServiceUtils.getDevicePixelRatio(target)
