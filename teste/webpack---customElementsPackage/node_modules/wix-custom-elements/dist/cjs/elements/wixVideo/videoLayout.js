...
var imageClientApi = require('image-client-api');
...
module.exports = {
     ...
     */
    mutate: function mutate(poster, canvas, videoNode, videoStyle, autoPlay, videoSourceUrl, needsSrcUpdate, animatePoster, videoFormat, isEditorMode) {
        dom.setAttributes(videoNode, {
            width: videoStyle.width,
            height: videoStyle.height
        });

        var notCanvas = !canvas;
        ...
        if (notCanvas) {
            ...
            dom.setStyle(videoNode, videoStyle);
        } else {
            dom.setStyle(canvas, videoStyle);
        }
...
function getVideoQualityBySize(qualities, _ref) {
    ...
    var uniqueQualities = _.unique(qualities, function(item) {
        return item.size;
    });
...
function getMP4Url(targetQuality, staticVideoUrl, videoId, videoFormat) {
    if (videoFormat === 'mp4') {
        ...
        if (targetQuality.url) {
            return _.joinURL(staticVideoUrl, targetQuality.url);
        }
        // build URL from videoId
        return _.joinURL(staticVideoUrl, videoId, targetQuality.quality, videoFormat, 'file.mp4');
    }
 ...
 * @param {boolean} isEditorMode
 */
function handlePosterVisibility(needsSrcUpdate, videoNode, posterNode, animatePoster, autoplay, isEditorMode) {
    ...
    if (needsSrcUpdate || isEditorMode) {
        if (autoplay) {
            videoNode.addEventListener('timeupdate', function handleTimeUpdate() {
            ...
            videoNode.addEventListener('play', function handlePlay() {
