...
function getTemplateValuesFromUrl(url) {
    var templates = [];
    var match = TEMPLATE_REG_EXP.exec(url);
    while (match) {
...
var uniqueCounter = 0; // eslint-disable-line santa/no-module-state
...
var persistentCacheKillerValue = null; // eslint-disable-line santa/no-module-state
...
function parseUrlParams(queryString) {
    ...
    while ((param = re.exec(queryString)) !== null) {
        var key = safeDecodeURIComponent(param[1]);
        var val = safeDecodeURIComponent(param[2]);
        if (!query[key]) {
...
function removeUrlParams(url, params) {
    ...
    delete urlObj.search;
    if (urlObj.query) {
        _.forEach(params, function(paramName) {
            delete urlObj.query[paramName];
        });
    }
...
module.exports = {
    ...
    setPersistentCacheKiller: setPersistentCacheKiller,
    persistentCacheKiller: persistentCacheKiller,
    resetPersistentCacheKiller: resetPersistentCacheKiller,
    ...
    getTemplateValuesFromUrl: getTemplateValuesFromUrl
};
