...
function render() {
    ...
    let value
    if (this._isAlive) {
        const currentT = _.now()
        this._tStep = currentT - this._t0
        this._t0 = currentT
        this._t += this._tStep
        for (let i = 0; i < this._easeParams.length; i++) {
...
function Tween(target, duration, params) {
    ...
    this._tStep = null
    this._isAlive = true
    const isElement = target instanceof window.HTMLElement
    ...
    const easeFuncName = parseParameter(params, 'ease', 'linear')
    ...
    let delay = parseParameter(params, 'delay', 0)
